...
function CreatePasswordForm(props) {
  ...
  const formatCheckPassword = (percent, successPercent) => {
    if (inRange(percent, 0, 50)) {
      return <FormattedMessage {...messages.passwordWeak} />;
    } else if (inRange(percent, 50, 80)) {
      return <FormattedMessage {...messages.passwordAverage} />;
    } else if (inRange(percent, 80, 100)) {
      return <FormattedMessage {...messages.passwordStrong} />;
    } else if (percent == 100) {
      return <FormattedMessage {...messages.passwordVeryStrong} />;
    }
  };
...
CreatePasswordForm.propTypes = {
  ...
  submitButtonName: PropTypes.object.isRequired,
};
